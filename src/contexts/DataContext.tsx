...
export function DataProvider({ children }: { children: ReactNode }) {
    ...
    if (IS_REAL_AUTH) {
      try {
        const dbId = await insertContaPagar({
          ...
          p_fk_conta_pai: newPayable.recurrenceParentId,
  ...
  const acceptEmailSuggestion = useCallback(async (id: string): Promise<AccountPayable | null> => {
    ...
    const newPayable: AccountPayable = {
      ...
      status: suggestion.suggestedStatus === 'PAGO' ? 'PAGO' : 'PENDENTE',
      paymentMethod: suggestion.suggestedPaymentMethod,
      paidAmount: suggestion.suggestedStatus === 'PAGO' ? suggestion.suggestedAmount : undefined,
      paidAt: suggestion.suggestedStatus === 'PAGO' ? (suggestion.suggestedPaidAt ?? now) : undefined,
      paidWith: suggestion.suggestedStatus === 'PAGO' ? suggestion.suggestedPaymentMethod : undefined,
      recurrence: 'NENHUMA',
      isUrgent: false,
